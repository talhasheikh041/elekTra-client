...
const DetailsSlider = ({ photos }: DetailsSliderProps) => {
   return (
      <Carousel>
         <CarouselContent>
            {photos.map((slide) => (
               <CarouselItem className="flex items-center justify-center" key={slide.public_id}>
                  <div className="">
                     <img
         ...
         <CarouselThumbButtons slides={photos.map((slide) => slide.url)} />
      </Carousel>
   )
}
